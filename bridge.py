...
async def _greeter_llm_client(prompt: str) -> str:
    """LLM adapter for ProactiveGreeter — stub.

    The ACP-backed implementation was retired with the rest of the
    ZeroClaw voice path in #36. ProactiveGreeter is now a no-op on the
    bridge side; the dotty-behaviour proactive-greeter consumer owns
    this path. Kept as an importable stub so anything still constructing
    a greeter doesn't NameError until that wiring is also ripped (#111
    commit 2)."""
    raise RuntimeError("greeter LLM client retired post-#36 — see dotty-behaviour")
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        _proactive_greeter = ProactiveGreeter(
            ...
            turn_logger=lambda **kw: None,  # convo logger retired #111 commit 1
        )
