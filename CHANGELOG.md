...
- **Child-safety enforcement sandwich** — five numbered rules in `STACKCHAN_TURN_SUFFIX` (audience framing for ages 4-8, forbidden-topic list, roleplay-lock, profanity-lock, ambiguity tie-breaker) injected at max-attention position for Qwen3 compliance. Tier 1 of a pre-designed four-tier lockdown plan.
