...
- Everything is LAN-local **except** the LLM call, which goes out to OpenRouter from the ZeroClaw host. EdgeTTS is cloud too when it's the selected TTS; Piper is fully local.
...
| **xiaozhi-esp32-server** | Docker host | VAD → ASR → LLM (proxy) → TTS pipeline, emotion dispatch, OTA | Docker container |
| **ZeroClawLLM custom provider** | Docker host (inside container) | Translates xiaozhi's LLM-provider interface to an HTTP POST | Python, mounted via volume |
| **zeroclaw-bridge** | ZeroClaw host | Accepts HTTP POSTs, spawns/holds a `zeroclaw acp` child, speaks ACP JSON-RPC to it | FastAPI + uvicorn under systemd |
...
sequenceDiagram
    ...
    participant Br as zeroclaw-bridge<br/>(ZeroClaw host)
    participant ZC as ZeroClaw<br/>(ZeroClaw host)
...
Paths and systemd unit names are env-configurable (`ZEROCLAW_VOICE_CFG`, `ZEROCLAW_VOICE_UNIT`, `ZEROCLAW_DISCORD_CFG`, `ZEROCLAW_DISCORD_UNIT`, `ZEROCLAW_WORKSPACE`); defaults match the documented ZeroClaw-host layout. The endpoints are off-the-shelf — if you don't reach `127.0.0.1:8080/admin/*`, they don't fire.
